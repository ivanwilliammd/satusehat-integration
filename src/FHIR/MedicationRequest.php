...
class MedicationRequest extends OAuth2Client
{
    ...
    public function __construct()
    {
        ...
        $this->medicationrequest_category = $medication_terminology->medication_requestdispense_category;
    ...
    public function addReasonCode($code, $display = null, $validation = true)
    {
        // If validation is true, check if the code is valid
        if ($validation) {
            $code_check = Icd10::where('icd10_code', $code)->first();

            // Handling if incomplete code / display
            if (! $code_check) {
                throw new FHIRInvalidPropertyValue('Kode ICD10 tidak ditemukan');
            }
    ...
    public function addDosageInstruction($sequence, $route_code, $timing_code, $patientInstruction, $as_needed = false, $dose_value = null, $dose_unit = null)
    {
        ...
        $dosage_instruction['timing']['code'] = [
            'coding' => [
                [
                    ...
                    'code' => $timing_code,
                    'display' => isset($this->timing[$timing_code]) ? $this->timing[$timing_code] : null,
                ],
            ],
        ];
    ...
    public function setDispenseRequest($quantity, $unit, $iter, $validityPeriodStart, $validityPeriodEnd, $expectedSupplyDurationNum, $expectedSupplyDurationUnit)
    {
        $dispense_request['numberOfRepeatsAllowed'] = $iter;

        $dispense_request['quantity']['value'] = $quantity;
        $dispense_request['quantity']['code'] = $unit;
        if (isset($this->drug_form[$unit])) {
            $dispense_request['quantity']['system'] = $this->drug_form[$unit]['system'];
            $dispense_request['quantity']['unit'] = $this->drug_form[$unit]['display'];
        } else {
            $dispense_request['quantity']['system'] = null;
            $dispense_request['quantity']['unit'] = null;
        }
        ...
        if (isset($expectedSupplyDurationUnit) && isset($expectedSupplyDurationNum)) {
            ...
            $dispense_request['expectedSupplyDuration']['unit'] = isset($this->time_interval[$expectedSupplyDurationUnit]) ? $this->time_interval[$expectedSupplyDurationUnit] : null;
        }
